...
HDScreen::HDScreen() : layoutPtr(nullptr), visible(false) {}
...
bool HDScreen::LoadLayout(const std::string &name) {
  ...
  // LayoutManager owns the layout, we just store a pointer to it
  // Store in our layout unique_ptr - we need to clone or just store raw pointer
  // For now, store as raw pointer since LayoutManager owns it
  layoutPtr = obj;
  printf("HD_UI: Loaded layout: %s (ptr=%p)\n", name.c_str(), (void *)obj);
  return true;
}
...
HD_MainMenu::HD_MainMenu() { layoutName = "MainMenu/MainMenu"; }
...
void HD_MainMenu::Remove() { HDScreen::Remove(); }

void HD_MainMenu::Update(float dt) { HDScreen::Update(dt); }
...
void HD_MainMenu::Draw() {
  ...
  Allegro5System::DrawGradientVertical(
      0, 0, (float)Allegro5System::GetWidth(),
      (float)Allegro5System::GetHeight(), 0.043f, 0.043f, 0.043f,
      1.0f,                          // Top: #0B0B0B
...
bool HD_MainMenu::HandleInput() { return HDScreen::HandleInput(); }

void HD_MainMenu::SetupCallbacks() {
  // Set up button callbacks
  UIObject *btnShutdown = FindElement("btnShutdown");
  if (btnShutdown) {
    btnShutdown->onClickCallback = [this](UIObject *) { OnShutdownClick(); };
  }

  UIObject *btnOptions = FindElement("btnOptions");
  if (btnOptions) {
    btnOptions->onClickCallback = [this](UIObject *) { OnOptionsClick(); };
  }

  UIObject *btnSubmit = FindElement("signIn/btnSubmit");
  if (btnSubmit) {
    btnSubmit->onClickCallback = [this](UIObject *) { OnPasswordSubmit(); };
  }
}
...
void HD_MainMenu::SetUsername(const std::string &name) {
  username = name;
  UITextPoint *txt =
      dynamic_cast<UITextPoint *>(FindElement("signIn/txtUsername"));
  if (txt) {
    txt->text = name;
  }
}
...
void HD_MainMenu::SetLastSignIn(const std::string &date) {
  UITextPoint *txt =
      dynamic_cast<UITextPoint *>(FindElement("signIn/txtLastSignIn"));
  if (txt) {
...
void HD_MainMenu::SetVersion(const std::string &version) {
  UITextPoint *txt =
      dynamic_cast<UITextPoint *>(FindElement("signIn/txtVersion"));
  if (txt) {
    txt->text = version;
  }
}
...
void HD_MainMenu::ShowError(const std::string &message) {
  UITextPoint *txt =
      dynamic_cast<UITextPoint *>(FindElement("signIn/txtNotice"));
  if (txt) {
    txt->text = message;
    txt->visible = true;
  }
...
HD_TopBar::HD_TopBar() : currentSpeed(1) { layoutName = "HUD/TopBar"; }
...
void HD_TopBar::Update(float dt) { HDScreen::Update(dt); }
...
void HD_TopBar::SetupCallbacks() {
  ...
  for (int i = 0; i <= 3; i++) {
    ...
    if (btn) {
      int speed = i;
      btn->onClickCallback = [this, speed](UIObject *) { SetSpeed(speed); };
    }
  }
}
...
void HD_TopBar::SetAccount(const std::string &accountNum, int balance) {
  UITextPoint *txt =
      dynamic_cast<UITextPoint *>(FindElement("btnAccount/txtCaption"));
  if (txt) {
    char buf[64];
...
void HD_TopBar::SetUsername(const std::string &name) {
  UITextPoint *txt =
      dynamic_cast<UITextPoint *>(FindElement("btnUser/txtCaption"));
  if (txt) {
    txt->text = name;
  }
}
...
HD_TaskBar::HD_TaskBar() { layoutName = "HUD/TaskBar"; }
...
void HD_TaskBar::Update(float dt) { HDScreen::Update(dt); }
...
void HD_TaskBar::SetEmailCount(int count) {
  UITextPoint *txt = dynamic_cast<UITextPoint *>(
      FindElement("btnEmail/notification/txtNumber"));
  if (txt) {
    char buf[8];
...
HDUIManager::HDUIManager() : hdModeActive(false) {}
...
void HDUIManager::Draw() {
  ...
  if (mainMenu)
    mainMenu->Draw(); // Force draw for testing
  if (topBar && topBar->IsVisible())
    topBar->Draw();
  if (taskBar && taskBar->IsVisible())
    taskBar->Draw();
